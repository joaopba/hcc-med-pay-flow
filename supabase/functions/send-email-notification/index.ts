...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'nova_nota') {
      ...
      const approveUrl = `${Deno.env.get('SUPABASE_URL')}/functions/v1/processar-aprovacao?nota=${notaId}&action=aprovar&token=${token}`;
      const rejectUrl = `${Deno.env.get('SUPABASE_URL')}/functions/v1/processar-aprovacao?nota=${notaId}&action=rejeitar&token=${token}`;
